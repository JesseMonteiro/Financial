...
export const PT_LABEL_TO_SUBCATEGORY_KEY: Record<string, string> = {
  "Supermercado & Alimentação": "Groceries",
  Supermercados: "Groceries",
  ...
  Loteria: "Gambling",
  "Aposta online": "Gambling",
  "Pensão alimentícia": "Legal obligations",
  "Saldos bloqueados": "Legal obligations",
};
...
export function canonicalBudgetCategory(raw: string | null | undefined): string {
  ...
  for (const [label, baseKey] of Object.entries(PT_LABEL_TO_BASE_KEY)) {
    if (label.toLowerCase() === lower && SUBCATEGORY_TO_PARENT[baseKey]) {
      return baseKey;
    }
  }
