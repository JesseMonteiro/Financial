...
public enum AppRoute: String, CaseIterable, Identifiable, Hashable, Sendable {
    ...
    public static let sidebarItems: [AppRoute] = [
        ...
        .manualExpenses, .categories, .subscriptions, .agenda, .goals, .mealVouchers, .reports,
...
public enum AppDeepLink: Equatable, Sendable {
    ...
    public static func parse(_ url: URL) -> AppDeepLink? {
        let host: String
        if url.scheme == "meuflux" {
            host = (url.host ?? url.path).trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        } else {
