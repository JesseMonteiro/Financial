...
public enum CardFaceCatalog {
    ...
    public static func style(
        ...
        return CardFaceStyle(
            assetName: nil,
            gradient: [MeuFluxColors.primary, MeuFluxColors.primaryHover],
...
public struct CreditCardFaceView: View {
    ...
    public var body: some View {
                ...
                Text(style.productLabel)
                    ...
                    if let status {
                        Text(status == .paid ? "Paga" : "A pagar")
                            ...
                            .background(
                                Capsule()
                                    .fill(status == .paid ? MeuFluxColors.success : MeuFluxColors.warning)
                            )
                    }
                }
        ...
        .overlay {
            RoundedRectangle(cornerRadius: CreditCardFaceMetrics.corner, style: .continuous)
                .strokeBorder(selected ? MeuFluxColors.primary : Color.clear, lineWidth: 2)
        }
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 8) {
                Image(systemName: "creditcard.fill")
                    .foregroundStyle(selected ? MeuFluxColors.primary : MeuFluxColors.textMuted)
                Text("Todos os Cartões")
                    .font(.caption.weight(.bold))
                    .foregroundStyle(selected ? MeuFluxColors.primary : MeuFluxColors.textPrimary)
            ...
            .background(
                selected ? MeuFluxColors.primary.opacity(0.12) : MeuFluxColors.bgTertiary,
                in: RoundedRectangle(cornerRadius: CreditCardFaceMetrics.corner, style: .continuous)
            )
            .overlay {
                RoundedRectangle(cornerRadius: CreditCardFaceMetrics.corner, style: .continuous)
                    .strokeBorder(selected ? MeuFluxColors.primary : MeuFluxColors.border, lineWidth: selected ? 2 : 1)
            }
        }
        .buttonStyle(.plain)
