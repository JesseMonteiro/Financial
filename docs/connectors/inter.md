...
- **Stub futuro sem lançamentos**: a Pluggy pode deixar uma fatura oficial à frente da aberta com `totalAmount` residual depois que as parcelas já foram para o ciclo aberto (Jesse nov/2026: oficial **R$ 50,67**, `updatedAt` 12/08, **0 txs**). `ignoreUnbackedOfficial` zera esse total e o chip some.
...
- **`hasSimilarInstallment` cross-key**: R$ 49,91 ativo não pode ser bloqueado por um 50,00 **já encerrado** no mesmo merchant (Lucas out/2026: faltava Redepharma 7/10 na aberta). Só trata como a mesma compra se `installmentSeriesAreAmountDriftTwins` concordar.
- **Compras paralelas idênticas**: duas Óticas Rocha 70,00 10× compartilham merchant+M+valor. Sem `purchaseDate` na chave da série, só uma 7/10 entra na aberta. Inter manda `purchaseDate` em toda parcela — usar na `installmentSeriesKey` (Lucas out/2026: 3× Nuuvem 16,66 faltando = **R$ 49,98** vs CSV do banco **R$ 4.795,50**).
